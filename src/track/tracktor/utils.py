...
def euclidean_distance(boxes, query_boxes):
    """
    Parameters
    ----------
    ...
    center_X1 = boxes[:, 0] + ((boxes[:,2] - boxes[:, 0] )/ 2 )
    center_X2 = query_boxes[:, 0] + ((query_boxes[:,2] - query_boxes[:, 0]) /2)
    center_Y1 = boxes[:, 1] + ((boxes[:,3] - boxes[:, 1] )/ 2 )
    center_Y2 = query_boxes[:, 1] + ((query_boxes[:,3] - query_boxes[:, 1]) /2)
    X = center_X1.view(-1,1) - center_X2.view(1,-1)
    XX = X**2
    Y = center_Y1.view(-1,1) - center_Y2.view(1,-1)
    YY = Y**2
    DD = XX + YY
    Distance = torch.sqrt(DD)

    return out_fn(Distance)
...
def bbox_overlaps(boxes, query_boxes):
    ...
    #print("areas", box_areas, query_areas)
    ...
    ua = box_areas.view(-1, 1) + query_areas.view(1, -1) - iw * ih #view : tensorのサイズを自動調整してくれる
    overlaps = iw * ih / ua
...
def evaluate_mot_accums(output_dir, accums, names, generate_overall=False):
