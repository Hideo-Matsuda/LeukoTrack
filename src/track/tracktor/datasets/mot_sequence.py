...
class MOTSequence(Dataset):
    ...
    def _sequence(self):
        ...
        if seq_name in self._test_folders:
            seq_path = osp.join(self._mot_dir, 'test', seq_name)
        elif seq_name in self._train_folders:
            ...
            with open(gt_file, "r") as inf:
                ...
                for row in reader:
                    ...
                    if int(row[6]) == 1 and int(row[7]) == 1 and float(row[8]) >= self._vis_threshold:
                        # Make pixel indexes 1-based, should already be 1-based (or not)
                        x1 = int(row[2])
                        y1 = int(row[3])
                        x2 = x1 + int(row[4])
                        y2 = y1 + int(row[5])
        ...
        for i in range(1, seqLength + 1):
            if 'biodata' in imDir:
                im_path = osp.join(imDir, f"{i:04d}.png") # for cell image Datasets
                #im_path = osp.join(imDir, f"{i:06d}.png") # for MOT Datasets
            else:
