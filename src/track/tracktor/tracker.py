...
class Tracker:
    ...
    def __init__(self, obj_detect, reid_network, tracker_cfg):
        ...
        self.reid_sim_threshold2 = tracker_cfg['reid_sim_threshold2']
        self.reid_iou_threshold = tracker_cfg['reid_iou_threshold']
        self.reid_distance_threshold = tracker_cfg['reid_distance_threshold']
    ...
    def reid(self, blob, new_det_pos, new_det_scores):
        ...
        if self.do_reid: 
            ...
            if len(self.inactive_tracks) >= 1:
                ...
                if self.reid_distance_threshold:
                    distance = euclidean_distance(pos, new_det_pos)
                    print("dis", distance)
                    distance_neg_mask = torch.ge(distance, self.reid_distance_threshold)
                    distance_mask = ~distance_neg_mask
                    dist_mat2 = dist_mat2 * distance_mask.float() * 0.001 + distance_neg_mask.float() * 1000
                    print("mat2", dist_mat2)
                dist_mat2 = dist_mat2.cpu().numpy()
                #exit(0)
                ...
                for r, c in zip(row_ind, col_ind):
                    if dist_mat[r, c] <= self.reid_sim_threshold or dist_mat2[r, c] <= self.reid_sim_threshold2:
    ...
    def step(self, blob): 
        ...
        # !!! Here NMS is used to filter out detections that are already covered by tracks.
        #  This is　done by iterating through the active tracks one by one, assigning them a bigger score
